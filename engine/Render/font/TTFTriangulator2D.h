...
template <typename TVert, typename TTri> class Triangulator2D {
	...
	protected:
		...
		static const size_t index_error = INT_MAX;
...
template <typename TVert, typename TTri> bool Triangulator2D<TVert, TTri>::SubdivideTri(size_t tri_index) {
	...
	if (t.coef == -1) {
		...
		if (ei0 == index_error || ei1 == index_error) return false;
		Edge& e0 = edges[ei0];
		Edge& e1 = edges[ei1];
		if (e0.ne != ei1 || e1.pe != ei0) return false;
	...
	if (t.coef == 1) {
		...
		if (ei == index_error) return false;
		Edge& e = edges[ei];
		Edge& ne = edges[e.ne];
