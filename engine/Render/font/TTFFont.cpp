...
void Font::VerifyTableCheckSums() const {
	for (auto i = table_map.begin(); i != table_map.end(); ++i) {
		if (VerifyTableCheckSum(i->second) == false) return;
	}
}
...
uint16_t Font::GetGlyphIndexF4(FItr itr, uint16_t langid, uint32_t code) const {
	...
	if (ReadUSHORT(end_codes + (seg_count - 1) * 2) != 0xffff) return 0;
	if (ReadUSHORT(end_codes + (seg_count) * 2) != 0) return 0;
...
Font::FRange Font::GetGlyphRange(uint16_t glyph_index) const {
	...
	// sanity check
	if (glyph_index >= num_glyps) return {};
...
FontMetrics Font::GetFontMetrics() const {
	...
	// sanity check
	if (version != 0x00010000) return {};
	if (metric_data_format != 0) return {};
