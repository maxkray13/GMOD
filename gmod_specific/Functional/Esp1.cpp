...
void cEsp::BaseEnt_Tick(C_BaseEntity * ent)
{
	...
	if (Engine::Var::Instance().Keypad_Logger.Get()) {
		...
		if (!only_update) {
			...
			//wchar_t Temp[256];
			float Delta = 18.f;//g_pSurface->GetFontTall(draw.font_esp) / 0.7272727272727273f;
			int idx = 1;


			auto meme = cColor(Engine::Var::Instance().Esp_Text_Color.Get());
			cColor curr = {};
			curr.SetColor(meme.b(), meme.g(), meme.r(), 255);

			//MultiByteToWideChar(CP_UTF8, NULL, &KeypadList[iter].TempCode[0], -1, &Temp[0], 255);
			Draw::Text_Outline(g_font, KeypadList[iter].TempCode.c_str(), (left + right) / 2, top + idx * Delta, FontSize, 2.f, curr.GetRawColor(), true);
			//draw.text((left + right) / 2, top + idx * Delta, Temp, draw.font_esp, Color(Engine::Var::Instance().Esp_Text_Color.Get()).to_cColor(), true);
			idx++;
			//	MultiByteToWideChar(CP_UTF8, NULL, &KeypadList[iter].StaticCode[0], -1, &Temp[0], 255);
			//	draw.text((left + right) / 2, top + idx * Delta, Temp, draw.font_esp, Color(Engine::Var::Instance().Esp_Text_Color.Get()).to_cColor(), true);
			Draw::Text_Outline(g_font, KeypadList[iter].StaticCode.c_str(), (left + right) / 2, top + idx * Delta, FontSize, 2.f, curr.GetRawColor(), true);
		}
...
void cEsp::DrawInfo(C_GMOD_Player * ply, player_info_t& info)
{
	...
	if (Engine::Var::Instance().Esp_Active_Weapon.Get()) {
		...
		if (!b_is_friend) {
			//Draw::Text_Outline(g_font, KeypadList[iter].TempCode.c_str(), (left + right)*0.5f, top + idx * Delta, FontSize, 2.f, Color(Engine::Var::Instance().Esp_Text_Color.Get()).GetRawColor());
			Draw::Text_Outline(g_font, weap_name, (left + right)*0.5f, bottom - Delta, FontSize, 2.f, b_use_team_col ? curr.GetRawColor() : curr3.GetRawColor(), true);
			//draw.text((left + right)*0.5f, bottom - Delta, Temp, draw.font_esp, b_use_team_col ? this->TeamColor : text_col, true);
		}
		else {
			Draw::Text_Outline(g_font, weap_name, (left + right)*0.5f, bottom - Delta, FontSize, 2.f, curr2.GetRawColor(), true);
	...
	if (Engine::Var::Instance().Esp_Distance.Get()) {
		...
		if (!b_is_friend) {
			...
			Draw::Text_Outline(g_font, Temp, (left + right)*0.5f, top + idx * Delta, FontSize, 2.f, b_use_team_col ? curr.GetRawColor() : curr3.GetRawColor(), true);

			//draw.text((left + right)*0.5f, top + idx * Delta, Temp, draw.font_esp, b_use_team_col ? this->TeamColor : text_col, true);
		}
		else {
			Draw::Text_Outline(g_font, Temp, (left + right)*0.5f, top + idx * Delta, FontSize, 2.f, curr2.GetRawColor(), true);
	...
	if (Engine::Var::Instance().Esp_Steam_Name.Get()) {
		...
		if (!b_is_friend) {
			Draw::Text_Outline(g_font, info.name, (left + right)*0.5f, top + idx * Delta, FontSize, 2.f, b_use_team_col ? curr.GetRawColor() : curr3.GetRawColor(), true);

			//draw.text((left + right)*0.5f, top + idx * Delta, Temp, draw.font_esp, b_use_team_col ? this->TeamColor : text_col, true);
		}
		else {
			Draw::Text_Outline(g_font, info.name, (left + right)*0.5f, top + idx * Delta, FontSize, 2.f, curr2.GetRawColor(), true);
	...
	if (Engine::Var::Instance().Esp_RP_Name.Get()) {
		...
		if (!b_is_friend) {
			Draw::Text_Outline(g_font, rp_name, (left + right)*0.5f, top + idx * Delta, FontSize, 2.f, b_use_team_col ? curr.GetRawColor() : curr3.GetRawColor(), true);
			//draw.text((left + right)*0.5f, top + idx * Delta, Temp, draw.font_esp, b_use_team_col ? this->TeamColor : text_col, true);
		}
		else {
			Draw::Text_Outline(g_font, rp_name, (left + right)*0.5f, top + idx * Delta, FontSize, 2.f, curr2.GetRawColor(), true);
			//draw.text((left + right)*0.5f, top + idx * Delta, Temp, draw.font_esp, Color(Engine::Var::Instance().Friend_Color.Get()).to_cColor(), true);
	...
	if (Engine::Var::Instance().Esp_Class.Get()) {
		...
		if (!b_is_friend) {
			Draw::Text_Outline(g_font, TeamName.c_str(), (left + right)*0.5f, top + idx * Delta, FontSize, 2.f, b_use_team_col ? curr.GetRawColor() : curr3.GetRawColor(), true);
			...
			Draw::Text_Outline(g_font, TeamName.c_str(), (left + right)*0.5f, top + idx * Delta, FontSize, 2.f, curr2.GetRawColor(), true);
			...
			if (var_bool->s_Name == ent_name) {
				if (var_bool->Get()) {
					...
					//	MultiByteToWideChar(CP_UTF8, NULL, &ent_name[0], -1, &Temp[0], 255);
						Draw::Text_Outline(g_font, ent_name.c_str(), (left + right) / 2, top + 0.5f, FontSize, 2.f, Color(Engine::Var::Instance().Esp_Text_Color.Get()).GetRawColor(), true);
	...
	if (Trace.m_pEnt&&Trace.m_pEnt->GetScriptName() && !IsBadReadPtr(Trace.m_pEnt->GetScriptName(), 0x4)) {
		...
		if (LastSelected == ent) {
			...
			if (Show >= 0.9) {
				...
				cColor curr2 = {};
				curr2.SetColor(meme2.b(), meme2.g(), meme2.r(), 255);
				Draw::Text_Outline(g_font, ent_name.c_str(), w / 2, h / 2, FontSize, 2.f, curr2.GetRawColor(), true);
