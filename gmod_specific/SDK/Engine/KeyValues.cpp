...
KeyValuess g_KeyValues;

void KeyValuess::Init() {

	DWORD ptr = (DWORD)Core::Memory::FindPattern((PBYTE)SDK::Client.GetStartAddr(), SDK::Client.GetSize(),
		(const char *)"\x8B\x4D\xFC\x8B\x45\xF8\x89\x41\x5C",//xxxxxxxxx
		0xCC);
	ptr += 0xB;
	this->Operator_NewPtr = (PVOID)Core::Memory::follow_to(ptr);


	ptr = (DWORD)Core::Memory::FindPattern((PBYTE)SDK::Client.GetStartAddr(), SDK::Client.GetSize(),
		(const char *)"\xE8\xCC\xCC\xCC\xCC\x8B\xF8\xEB\x02\x33\xFF\xFF\x75\xF4",//x????xxxxxxxxx
		0xCC);
//	ptr = ptr + *(DWORD*)ptr +0x4;
	this->ConstructorPtr = (PVOID)Core::Memory::follow_to(ptr);
	
	

	ptr = (DWORD)Core::Memory::FindPattern((PBYTE)SDK::Client.GetStartAddr(), SDK::Client.GetSize(),
		(const char *)"\xE8\xCC\xCC\xCC\xCC\x8B\x0D\xCC\xCC\xCC\xCC\x57\x8B\x01\xFF\x90",//x????xx????xxxxx
		0xCC);
//	ptr = ptr + *(DWORD*)ptr + 0x4;
	this->SetIntPtr = (PVOID)Core::Memory::follow_to(ptr);

	this->LoadFromBufferPtr = (PVOID)Core::Memory::FindPattern((PBYTE)SDK::Client.GetStartAddr(), SDK::Client.GetSize(),
		(const char *)"\x55\x8B\xEC\x83\xEC\x34\xCC\x8B\xCC\xCC\x89\xCC\xCC\x85",
		0xCC);
	//
/*	this->SetIntPtr = *(KeyValues_SetInt*)((DWORD)(mTools.FindPattern((PBYTE)cGMOD_hooks.Client.GetStartAddr(), cGMOD_hooks.Client.GetSize(),
		(const char *)"\x68\xCC\xCC\xCC\xCC\xE8\xCC\xCC\xCC\xCC\x8B\x0D\xCC\xCC\xCC\xCC\x57",//x????x????xx????x
		0xCC)) + 0x1);*/

}
PVOID KeyValuess::Operator_New() {
	PVOID RET;
	PVOID ADDR = (PVOID)Operator_NewPtr;
	_asm {
		push 20h;
		call ADDR;
		add esp, 4;
...
PVOID KeyValuess::Constructor(PVOID kv, const char * command) {
	PVOID RET;
	PVOID ADDR = (PVOID)this->ConstructorPtr;
	_asm { 
		push command;
		mov ecx, kv;
		call ADDR;
...
void KeyValuess::SetInt(PVOID kv, const char * Name, int Value) {
	PVOID ADDR = (PVOID)this->SetIntPtr;
	_asm {
		mov ecx, kv;
		push Value;
		push Name;
...
bool KeyValuess::LoadFromBuffer(PVOID kv, const char * resourceName, const char * pBuffer, void * pFileSystem, const char * pPathID)
{
	using t = bool(__thiscall*)(void*kv,const char * resourceName, const char * pBuffer, void * pFileSystem, const char * pPathID);

	return ((t)LoadFromBufferPtr)(kv, resourceName, pBuffer, pFileSystem, pPathID);
}
#define Shit 0
#ifndef Shit
...
bool CKeyValuesDumpContextAsDevMsg::KvWriteText(char const *szText)
{
	...
	{
	//	printf_s(m_nDeveloperLevel, "%s", szText);
	}
	else
	{
		printf_s("%s", szText);
